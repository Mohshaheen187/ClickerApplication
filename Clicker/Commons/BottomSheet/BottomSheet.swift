    ...
    public var body: some View {
        GeometryReader { geometry in
            ZStack {
                ...
                .frame(width :  UIScreen.main.bounds.width ,height: self.height - min(self.draggedOffset*2, 0))
                .background(self.contentBackgroundColor)
                .cornerRadius(self.topBarCornerRadius, corners: [.topLeft, .topRight])
                .animation(.interactiveSpring())
                .offset(y: self.isPresented ? (geometry.size.height - (self.height * 0.7) + geometry.safeAreaInsets.bottom + self.draggedOffset) : (geometry.size.height/2 + (self.height * 0.9) + geometry.safeAreaInsets.bottom))
            }
        }
    }
